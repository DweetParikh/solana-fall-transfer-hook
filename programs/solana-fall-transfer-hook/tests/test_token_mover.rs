...
#[test]
fn test_program_transfer_succeeds() {
    ...
    assert!(res.is_ok(), "Program-initiated transfer failed: {:?}", res.err());
}

#[test]
fn test_program_transfer_respects_rate_limit() {
    let (mut svm, payer, program_id) = setup();
    let mint = Keypair::new();
    ...
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&payer]).unwrap();
    assert!(svm.send_transaction(tx).is_ok(), "transfer at limit should succeed");

    // One token more — should fail with RateLimitExceeded (0x1771)
    ...
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&payer]).unwrap();
    assert!(svm.send_transaction(tx).is_err(), "transfer exceeding rate limit should fail");
}