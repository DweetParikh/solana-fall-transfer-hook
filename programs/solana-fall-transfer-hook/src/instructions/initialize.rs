...
pub struct Initialize<'info> {
    ...
    pub mint: InterfaceAccount<'info, Mint>,        // validating the mint
    #[account(
        init,
        payer = payer,
        ...
        seeds = [b"rate_limit", mint.key().as_ref(), payer.key().as_ref()],     // Unnique rate-limit PDA for mint payer pair
        bump,
...
pub fn handler(ctx: Context<Initialize>) -> Result<()> {
    ...
    ctx.accounts.rate_limit.set_inner(RateLimit {
        mint: ctx.accounts.mint.key(),      // STores theh mint
